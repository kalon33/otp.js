...
/// TRANSLATORS: Optimization for bicycle shown in bike triangle. Optimized for bike friendly infrastructure. Cycle roads etc...
  ...
  render: function() {
    ...
    this.quickBar = canvas.rect(barLeft, margin, barWidth * 0.333,
      barHeight);
    this.quickBar.attr({
      fill: quickFill,
      stroke: 'none'
    ...
    this.flatBar = canvas.rect(barLeft, margin * 2 + barHeight, barWidth *
      0.333, barHeight);
    this.flatBar.attr({
      fill: flatFill,
      stroke: 'none'
    ...
    this.safeBar = canvas.rect(barLeft, margin * 3 + barHeight * 2,
      barWidth * 0.333, barHeight);
    this.safeBar.attr({
      fill: safeFill,
      stroke: 'none'
    });

    this.quickLabel = canvas.text(barLeft + margin, margin + barHeight / 2,
      this.quickName + ': 33%');
    ...
    this.flatLabel = canvas.text(barLeft + margin, margin * 2 + barHeight +
      barHeight / 2, this.flatName + ': 33%');
    this.flatLabel.attr({
      'font-size': '13px',
      'text-anchor': 'start',
      opacity: 1
    ...
    this.safeLabel = canvas.text(barLeft + margin, margin * 3 + barHeight *
      2 + barHeight / 2, this.safeName + ': 33%');
    this.safeLabel.attr({
      'font-size': '13px',
      'text-anchor': 'start',
      opacity: 1
    ...
    var cx = margin + tri_side / 2,
      cy = height - margin - (1 / Math.sqrt(3)) * (tri_side / 2);
    this.cursorVert = canvas.rect(cx - 0.5, cy - this.cursor_size / 2 - 2,
      1, this.cursor_size + 4).attr({
      fill: 'rgb(0,0,0)',
      stroke: 'none'
    });
    this.cursorHoriz = canvas.rect(cx - this.cursor_size / 2 - 2, cy - 0.5,
      this.cursor_size + 4, 1).attr({
      fill: 'rgb(0,0,0)',
      stroke: 'none'
    });
    ...
    var start = function() {
        ...
        this_.quickBar.animate({
          opacity: 0.25
        }, animTime);
        this_.flatBar.animateWith(this_.quickBar, {
          opacity: 0.25
        }, animTime);
        this_.safeBar.animateWith(this_.quickBar, {
          opacity: 0.25
        }, animTime);
      },
      move = function(dx, dy) {
        ...
        if (nx < margin + (tri_side / 2) - offset) nx = margin + (tri_side /
          2) - offset;
        if (nx > margin + (tri_side / 2) + offset) nx = margin + (tri_side /
          2) + offset;

        time = ((height - 2 * margin) - (ny - margin)) / (height - 2 *
          margin);
        topo = this_.distToSegment(nx, ny, margin + tri_side / 2, margin,
          margin + tri_side, height - margin) / (height - 2 * margin);
        safety = 1 - time - topo;
        ...
        this_.quickLabel.attr('text', this_.quickName + ': ' + Math.round(
          time * 100) + '%');
        this_.flatLabel.attr('text', this_.flatName + ': ' + Math.round(
          topo * 100) + '%');
        this_.safeLabel.attr('text', this_.safeName + ': ' + Math.round(
          safety * 100) + '%');
  ...
  distToSegment: function(px, py, x1, y1, x2, y2) {
    ...
    return this.distance(px, py, (1 - r) * x1 + r * x2, (1 - r) * y1 + r *
      y2);
  },
  ...
  setValues: function(quick, flat, safe) {
    ...
    this.quickLabel.attr('text', this.quickName + ': ' + Math.round(quick *
      100) + '%');
    this.flatLabel.attr('text', this.flatName + ': ' + Math.round(flat *
      100) + '%');
    this.safeLabel.attr('text', this.safeName + ': ' + Math.round(safe *
      100) + '%');
    ...
    var qx = 0,
      qy = -this.tri_side / Math.sqrt(3);
    var fx = -this.tri_side / 2,
      fy = (this.tri_side / 2) / Math.sqrt(3);
    var sx = this.tri_side / 2,
      sy = (this.tri_side / 2) / Math.sqrt(3);
